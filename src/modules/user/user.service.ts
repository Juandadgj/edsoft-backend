...
export class UserService {
  ...
  async signIn(signInInput: SignInInput) {
    ...
    const userDB = await clients[DB].user.findFirst({where: {user: signInInput.user}})
    if (!userDB) throw new Error('User not found')
    if (signInInput.password != userDB.password) throw new Error('Incorrect password')
  ...
  async update(id_institution: string, updateUserInput: UpdateUserInput) {
    return await clients[id_institution].user.update({
