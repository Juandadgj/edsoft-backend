  ...
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
      ...
      <body>
        ...
        <div>
        ${achievements.map((achievement: any, i: number) => (`<p> <b>${i + 1}.</b> ${achievement.description}</p>`)).join('')}
        </div>
        <br/>
        <table>
          <thead>
            <tr>
              ...
              ${achievements.map((_: any, i: number) => (`<th>${i + 1}</th>`)).join('')}
              <th>GL</th>
            </tr>
          </thead>
          <tbody>
    ...
    return (
      `<tr>
                    <td>${i + 1}</td>
                    <td class="name_td">${student.last_name} ${student.name}</td>
